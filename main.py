...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:GARUD@localhost/covid'
db=SQLAlchemy(app)
...
class Hospitaldata(db.Model):
    id=db.Column(db.Integer,primary_key=True)
    hcode=db.Column(db.String(20),db.ForeignKey(Hospitaluser.hcode))
...
def signup():
    if request.method=="POST":
        ...
        flash("SignUp Success Email Is Sent Please Login","success")
...
def logoutadmin():
    session.pop('user')
    flash("You logged out admin", "primary")
